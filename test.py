...
from utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, non_max_suppression_gaussianobb, scale_coords, scale_polys, xyxy2xywh, xywh2xyxy, set_logging, increment_path, colorstr
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            t = time_synchronized()
            out = non_max_suppression_gaussianobb(out, conf_thres=conf_thres, iou_thres=iou_thres, labels=lb, multi_label=True)
            # [ xywh theta conf cls ]
        ...
        for si, pred in enumerate(out):
            ...
            correct = torch.zeros(pred_hbb.shape[0], niou, dtype=torch.bool, device=device)
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    pi = (cls == pred_hbb[:, 5]).nonzero(as_tuple=False).view(-1)  # target indices
            ...
            stats.append((correct.cpu(), pred_hbb[:, 4].cpu(), pred_hbb[:, 5].cpu(), tcls))
